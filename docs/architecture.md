...
Реализован проверяемый backend-сервис, который принимает CSV-отчёты с остатками, сохраняет оригиналы в MinIO, валидирует данные и атомарно синхронизирует остатки в PostgreSQL.
...
Документ фиксирует фактическую архитектуру завершённой реализации. Инструкции запуска и проверки находятся в `README.md`.
...
- Оригинальный файл сохраняется и доступен для любого зарегистрированного статуса.
...
5. при успешной проверке оставляет отчёт в `processing`, а на следующем polling-цикле запускает одну apply-транзакцию;
6. при validation error сохраняет `failed` без запуска apply;
7. после терминального статуса очищает staging отчёта.

Если процесс упал со статусом `processing`, новый worker после получения lock проверяет полноту validation. Полностью сформированный staging передаётся в apply; незавершённый staging и ошибки предыдущей попытки удаляются, после чего оригинал читается заново. Более новый отчёт не обгоняет прерванный.
...
| `checksum_sha256` | `VARCHAR(64)` | Потоково вычисленный SHA-256 оригинала |
...
- `0001_initial_schema` создаёт отчёты, staging/errors и предметные таблицы;
- `0002_add_report_checksum` добавляет `reports.checksum_sha256` и CHECK формата.

Текущая migration head — `0002_add_report_checksum`; `0001` после применения не переписывается.
...
- Lock удерживается до завершения текущего polling-цикла. Потоковая validation и apply валидного staging выполняются в двух последовательных циклах; FIFO не позволяет более новому отчёту пройти между ними.
