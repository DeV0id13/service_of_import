...
Все этапы плана завершены. Документ сохранён как история решений и карта фактической реализации; актуальные команды запуска находятся в `README.md`.
...
│       ├── csv_validation.py   # потоковый CSV parser/validation
│       ├── report_processing.py # FIFO orchestration и staging/errors
│       ├── read_api.py         # read-модели и запросы каталогов/остатков
...
### Этап 1. Каркас и локальная инфраструктура — завершён
...
### Этап 2. Модель данных и Alembic — завершён
...
### Этап 3. Загрузка и API отчётов — завершён
...
### Этап 4. FIFO worker, validation и staging — завершён
...
### Этап 5. Атомарное применение — завершён
...
### Этап 6. Read API, документация и финальная проверка — завершён
...
Фактическая цепочка содержит две migration:

1. `0001_initial_schema` создаёт исходную схему;
2. `0002_add_report_checksum` добавляет потоковый SHA-256 оригинала.
...
`0001` не переписывался после появления следующей migration. Текущая head — `0002_add_report_checksum`; последующие изменения должны оформляться новыми revision.

### Фактические отклонения от первоначального плана

- checksum был добавлен отдельной migration `0002`, а не включён задним числом в `0001`;
- object key формируется через UUID до регистрации отчёта, поэтому не зависит от пользовательского filename;
- полностью провалидированный отчёт остаётся `processing` и применяется на следующем polling-цикле под тем же FIFO-правилом;
- вместо предполагаемого `validation.py` используются отдельные `csv_validation.py` и `report_processing.py`;
- end-to-end handoff реализован переносимым `scripts/smoke.sh`, а не отдельным pytest-каталогом `tests/e2e`.
...
## 8. Итоговая готовность реализации
